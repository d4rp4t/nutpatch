...
export function assertSigFlag(flag: string): void {
  if (!VALID_SIG_FLAGS.has(flag))
    throw new Error(
      `Invalid sigflag "${flag}": must be "SIG_INPUTS" or "SIG_ALL"`
    )
}
...
export function assertSpendingConditionRules(
  ...
  if (nSigsRefund !== undefined) {
    assertPositiveInteger(
      nSigsRefund,
      'requiredRefundSignatures (n_sigs_refund)'
    )
    if (refundKeyCount === 0)
      throw new Error(
        'requiredRefundSignatures (n_sigs_refund) requires refund keys'
      )
...
export function normalizeP2PKOptions(p2pk: P2PKOptions): NormalizedP2PK {
  const pubkeys = dedupeP2PKPubkeys(
    Array.isArray(p2pk.pubkey) ? p2pk.pubkey : [p2pk.pubkey]
  )
  ...
  if (totalKeys > 10)
    throw new Error(
      `Too many pubkeys, ${totalKeys} provided, maximum allowed is 10 in total`
    )
  ...
  return {
    ...
    requiredRefundSignatures:
      requiredRefundSignatures !== undefined && requiredRefundSignatures > 1
        ? requiredRefundSignatures
        : undefined,
    additionalTags: p2pk.additionalTags?.length
      ? p2pk.additionalTags
      : undefined,
